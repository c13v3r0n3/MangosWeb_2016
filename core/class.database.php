...
/************************************************************
* Creates the connection to the mysqli database, selects the posted DB
...
*/
    ...
    {
        $this->mysqli = @mysqli_connect($db_host, $db_user, $db_pass, $db_name, $db_port);
        $this->selected_database = @mysqli_select_db($this->mysqli, $db_name);
		return TRUE;
    }

//	************************************************************
// Closes the mysqli DB connection

    public function __destruct()
    {
        @mysqli_close($this->mysqli) or die(mysqli_error($this->mysqli));
    }

/************************************************************
* Checks the connection to the mysqli database, selects the posted DB
* Returns 0 if unable to connect to the database
* Returns 2 if the Database does not exist
* Returns TRUE on success
*/
...
// Query function is best used for INSERT and UPDATE functions
    ...
    {
        $sql = mysqli_query($this->mysqli, $query) or die("Couldnt Run Query: ".$query."<br />Error: ".mysqli_error($this->mysqli)."");
...
// $file is the path location to the sql file
	...
	function runSQL($file)
	{
		...
		foreach($sql_line as $query)
		{
			if($query)
			{
				mysqli_query($query, $this->mysqli) or die("Couldnt Run Query: ".$query."<br />Error: ".mysqli_error($this->mysqli)."");
			}
		}
		return TRUE;
	}
